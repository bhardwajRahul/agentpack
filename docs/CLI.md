...
For `codex`, `claude`, and `cursor`, the default install includes a builder and
brief guidance for optional, announced use. Existing Claude/Cursor builders are
preserved; Codex updates only a managed block. Claude Desktop and git-hooks do
not install builders. See [integration details](INTEGRATIONS.md#optional-builder)
for usage and reinstall behavior.
...
`task audit [--json]` checks the current passport for branch/head drift, missing next actions, open verification, closed-task anomalies, source-cache metadata drift, and advisory-only risk-proportional verification evidence. For low risk, a short readable note or useful test output should say what was checked, the result, and any relevant limit. It has no required labels or minimum length, though empty, head-only, unreadable, or wholly generic evidence is flagged. Code scopes include a `Reviewed HEAD` exactly matching the Passport-bound SHA. Medium/high risk retains independent read-only review and a named negative/differential/operational/rollback check. Contract-changing work should be classified medium/high. After risk calibration, incomplete referenced evidence produces exact missing or malformed labels for the best incomplete candidate and a copy-ready template; the advisory neither judges technical correctness nor blocks. Metadata warnings are shown separately so they do not look like action-required task failures. Its additive `ceremonyDiagnostics` field only reports bounded review candidates, never lifecycle violations or claimed savings.
